...
public class PopulateDB {
    ...
    public void addQuestionsFromFile() throws IOException {
        ...
        for(Map.Entry<String,GameMode> entry: Constants.QA_FILES.entrySet()){
            ...
            for(Pair<String,String> question_answers: Utils.readQAFile(filename))
            {
                Question q = new Question();
                q.setQuestionText(question_answers.getFirst());
                q.setCorrectAnswer(question_answers.getSecond());
                q.setGameMode(gameMode);
                questionRepository.save(q);
                questionNumber ++;
                if(questionNumber > Constants.MAX_QUESTIONS_T0_READ)
                    break;
            }
        }
    }
